...
<form action="" method="get">
	<div class="input-group col-md-5 col-md-offset-7">
		...
		<select type="submit" name="tanggal" class="form-control" onchange="this.form.submit()">
			...
			<?php 
			$pil=xsql("SELECT distinct tanggal from barang_laku order by tanggal desc",$x);
			while($p=xfetch_a($pil)){
				?>
...
if(isset($_GET['tanggal'])){
	$tanggal=mysqli_real_escape_string($x, $_GET['tanggal']);
	$tg="proses/p_laporan_entry.php?tanggal='$tanggal'";
...
<table class="table">
	...
	if(isset($_GET['tanggal'])){
		$tanggal=mysqli_real_escape_string($x,$_GET['tanggal']);
		$brg=xsql("SELECT * from barang_laku where tanggal like '$tanggal' order by tanggal desc",$x);
	}else{
		$brg=xsql("SELECT * from barang_laku order by tanggal desc",$x);
	}
	$no=1;
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal=mysqli_real_escape_string($x, $_GET['tanggal']);
			$tanggalsql=xsql("SELECT sum(total_harga) as total from barang_laku where tanggal='$tanggal'",$x);	
			$tanggalrs=xfetch_a($tanggalsql);			
			echo "<td><b> Rp.". Rp($tanggalrs['total']).",-</b></td>";
		}else{
	...
	<tr>
		...
		if(isset($_GET['tanggal'])){
			$tanggal=mysqli_real_escape_string($x, $_GET['tanggal']);
			$tanggalsql=xsql("SELECT sum(laba) as total from barang_laku where tanggal='$tanggal'",$x);	
			$tanggalrs=xfetch_a($tanggalsql);			
			echo "<td><b> Rp.". Rp($tanggalrs['total']).",-</b></td>";
		}else{
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<div class="modal-body">				
					<form action="proses/p_tambah_entry.php" method="post">
